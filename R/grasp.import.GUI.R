...
function () 
{
# GUI for data import in R-GRASP
...
opendlgXXX <- function(...)
{
    ...
    XXX.local <- read.delim(file = file.select.XXX)
    cat("Responses saved into XXX.local", "\n")
    assign("XXX.local", XXX.local, pos = 1)
    label2.full <- paste("XXX saved from ", file.select.XXX, sep = " ")
...
opendlgXXXpred <- function(...)
{
    ...
    label3.full <- paste("XXXpred saved from ", file.select.XXXpred, sep = " ")
...
apply.import <- function(...)
{
    ...
    cat("XXXpred created...", "\n")
    label.applied <- paste("Import done for:", label.applied, sep = " ")
...
frame2.but1 <- tkbutton(frame2, text = "Import...", padx = 20, command = opendlgYYY)
frame2.but2 <- tkbutton(frame2, text = "import...", padx = 20, command = opendlgXXX)
frame2.but3 <- tkbutton(frame2, text = "import...", padx = 20, command = opendlgXXXpred)
...
apply.but.import <- tkbutton(frame4, text = "Apply", padx = 30, command = apply.import)
close.but.import <- tkbutton(frame4, text = "Close", padx = 30, command = destroy.import)
