...
function(gr.Yi, sX = gr.selX, nbar = 10)
{
    ...
    x11()
    if (is.null(nbar))
        stop("You must provide the numbers of histogram bars!")
    ...
    for(i in sX) {
        if(is.numeric(XX[, i])) {
            #Xmin <- round(min(XX[, i]),2)
	    Xmin <- round(min(XX[!is.na(XX[, i]), i]), 2)
            #Xlag <- round((max(XX[,i])-Xmin)/nbar)
	    Xlag <- (max(XX[!is.na(XX[, i]), i]) - Xmin)/nbar
            ...
            temp <- table(factor(cut(y[!is.na(XX[, i])], c(-1, 1e-005, 500))), cut(XX[!is.na(XX[, i]), i], nbar))
            loc <- barplot(temp, xlab = names(XX[i]), ylab = "count", names = Xbreaks)
            #if(prop < 1) {
            #    text(loc, apply(temp, 2, sum) + par("cxy")[2], temp[2,  ])
            #}
            ...
            if(prop < 1) {
                ...
                barplot(fred2, axes = FALSE, axisnames = FALSE, density = -1, space = 0, lwd = 0.5, col = grey(0))
                abline(h = 1, lty = 2)
            }
            if (first) {
                title("GraspeR histograms")
                first <- FALSE
            }
        }
