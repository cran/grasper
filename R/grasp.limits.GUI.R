...
apply.limits <- function()
{
    ...
    frame1.label1 <- tklabel(frame1, text = "Set limits", justify = "left", wraplength = 200, font = "Arial 11")
    tkgrid(frame1.label1, columnspan = 2)
    frame1.label2 <- tklabel(frame1, text = "This function restricts the data within limits defined for the below selected response variables (min. 2) by keeping the below given number of zeroes observations on each side of the last presence along selected predictors.", font = "Arial 9", wraplength = 200)
